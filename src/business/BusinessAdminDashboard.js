...
export default function BusinessAdminDashboard({ user, profile, categories = [] }) {
  ...
  if (panel === 'approvals') return <ScrollView contentContainerStyle={styles.content}><BusinessApprovalPanel mode="approvals" onBack={() => setPanel('overview')} /></ScrollView>;
  if (panel === 'businesses') return <ScrollView contentContainerStyle={styles.content}><BusinessApprovalPanel mode="management" onBack={() => setPanel('overview')} /></ScrollView>;
